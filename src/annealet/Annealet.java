/**
 * Uses the Metropolis algorithm to accomplish simulated annealing.
 */
...
public class Annealet
{
	...
	private boolean attemptJump() throws Exception
	{
		double res1;
		double res2;
	...
	private boolean consultMetropolis(double T, double deltaEnergy)
	{
		...
		if (deltaEnergy < 0.)
			return true;
		if (Math.exp(-deltaEnergy / T) > r.nextDouble())
			return true;
		return false;
	}

	protected boolean continueAnnealing() throws Exception
	{
